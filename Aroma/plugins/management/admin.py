...
async def demote_user(client, message):
    ...
    if user_member.privileges.can_promote_members and not user_member.privileges.can_restrict_members:
        await client.send_message(chat_id, "ʏᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴅᴇᴍᴏᴛᴇ ʀɪɢʜᴛꜱ.")
        return
    ...
    if target_member.privileges and target_member.privileges.can_promote_members:
        promoted_by = target_member.privileges.promoted_by if hasattr(target_member.privileges, 'promoted_by') else "unknown"
        await client.send_message(chat_id, f"{target_member.user.first_name} ᴄᴀɴ'ᴛ ʙᴇ ᴅᴇᴍᴏᴛᴇᴅ. ᴛʜᴇʏ ᴡᴇʀᴇ ᴘʀᴏᴍᴏᴛᴇᴅ ʙʏ {promoted_by}.")
        return
