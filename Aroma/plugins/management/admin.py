...
async def demote_user(client, message):
    ...
    user_member = await client.get_chat_member(chat_id, message.from_user.id)
    if not user_member.privileges or not user_member.privileges.can_promote_members:
        await client.send_message(chat_id, "ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀɴ ᴀᴅᴍɪɴ ᴏʀ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴘᴇʀᴍɪꜱꜱɪᴏɴ ᴛᴏ ᴅᴇᴍᴏᴛᴇ ᴜꜱᴇʀꜱ.")
        return
    ...
    try:
        ...
        await client.send_message(chat_id, "ᴛʜᴇ ᴜꜱᴇʀ ɪs ɴᴏᴛ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ.")
        return

    if target_user_id == message.from_user.id:
        await client.send_message(chat_id, "ʏᴏᴜ ᴄᴀɴ'ᴛ ᴅᴇᴍᴏᴛᴇ ʏᴏᴜʀsᴇʟғ.")
        return

    if not target_member.privileges or not target_member.privileges.can_promote_members:
        await client.send_message(chat_id, "ᴛʜᴇ ᴜꜱᴇʀ ɪs ᴀʟʀᴇᴀᴅʏ ᴀ ᴍᴇᴍʙᴇʀ.")
        return

    # Check if promoted by another bot
    if target_member.promoted_by and target_member.promoted_by.user.id != bot_user.id:
        await client.send_message(chat_id, "ᴛʜᴇ ᴜꜱᴇʀ ᴡᴀs ᴘʀᴏᴍᴏᴛᴇᴅ ʙʏ ᴀɴᴏᴛʜᴇʀ ʙᴏᴛ, ᴄᴀɴ'ᴛ ᴅᴇᴍᴏᴛᴇ.")
        return
