...
async def unban_user(client, message):
    ...
    if not user_member.privileges.can_restrict_members:
        await client.send_message(chat_id, "You don't have rights to unban this user.")
        return
    ...
    if target_user_id == bot_user.id:
        await client.send_message(chat_id, "Seriously, I'm not gonna unban myself.")
        return
    ...
    try:
        ...
        if target_user_member.status != ChatMemberStatus.BANNED:
            await client.send_message(chat_id, "User is not banned.")
            return
    except Exception:
    ...
    try:
        ...
        notification_message = f"A user has been unbanned in chat:\nUser: {target_name}\nUnbanned by: {admin_name}"
        await client.send_message(chat_id, notification_message)
    except Exception as e:
        await client.send_message(chat_id, f"Failed to unban user: {str(e)}")
