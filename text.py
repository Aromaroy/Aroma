...
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, ChatPrivileges, ChatPermissions, Message
...
async def promote_user(client, message):
    ...
    try:
        ...
        if bot_member.status != ChatMemberStatus.ADMINISTRATOR:
            await client.send_message(chat_id, "I am not an admin in this group.")
            return
    ...
    if user_member.status not in [ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER]:
        await client.send_message(chat_id, "You are not an admin or the owner to promote someone.")
        return

    if user_member.status == ChatMemberStatus.ADMINISTRATOR and not user_member.privileges.can_promote_members:
        await client.send_message(chat_id, "You don't have rights to promote users in this group.")
        return
    ...
    if target_user_id == bot_user.id:
        await client.send_message(chat_id, "I cannot promote myself.")
        return

    if target_user_id == message.from_user.id:
        await client.send_message(chat_id, "You can't issue the command for yourself.")
        return

    chat_owner = await client.get_chat_member(chat_id, chat_id)

    if target_user_id == chat_owner.user.id:
        await client.send_message(chat_id, "Duh, you want to promote the owner??")
        return

    try:
        target_member = await client.get_chat_member(chat_id, target_user_id)
    except UserNotParticipant:
        await client.send_message(chat_id, "The target user is not a member of this group.")
        logger.error("Target user is not a participant in the group.")
        return
    except Exception as e:
        if 'USER_NOT_PARTICIPANT' in str(e):
            await client.send_message(chat_id, "The target user is not a member of this group.")
        else:
            await client.send_message(chat_id, f"Error checking target user status: {e}")
        logger.error(f"Error checking target user status: {e}")
        return
    ...
    if target_member.status == ChatMemberStatus.ADMINISTRATOR:
        ...
        if promoted_by is None or promoted_by != bot_user.id:
            await client.send_message(chat_id, f"The user is already an admin")
            return
...
async def get_target_user_id(client, chat_id, message):
    ...
    if len(message.command) > 1:
        ...
        await client.send_message(chat_id, "ᴘʟᴇᴀꜱᴇ ꜱᴘᴇᴀꜱᴇ ᴀ ᴜꜱᴇʀ ᴛᴏ ᴘʀᴏᴍᴏᴛᴇ ʙʏ ᴜꜱᴇʀɴᴀᴍᴇ, ᴜꜱᴇʀ ɪᴅ, ᴏʀ ʀᴇᴘʟʏɪɴɢ ᴛᴏ ᴛʜᴇɪʀ ᴍᴇꜱꜱᴀɢᴇ..")
        return None
